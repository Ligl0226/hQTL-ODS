...
BPH_hQTL_ODS <- function(PHdata, GNdata, KinMatlist=NULL, P3D=TRUE){
  ...
  Sys.time()
  ##==================================================================##
  ##--Generate the transition matrix (Tmat) from trait to BPH
  ##==================================================================##
  cat("\n##===Generate the transition matrix from trait to BPH===##\n")
  rownames(PHdata) <- PHdata$Genotype
  ngen <- nrow(PHdata)
  ...
  for(i in 1:nhybrid){
    ...
    femalei_value <- PHdata[femalei,TraitName]
    malei_value <- PHdata[malei,TraitName]
    
    if(femalei_value>=malei_value){
      Tmat[i,femalei] <- -1
      # Tmat[i,malei] <- 0
    }else{
      # Tmat[i,femalei] <- 0
  ...
  all(Tmat %*% as.matrix(Adata[,1])==0)           ##Tmiai!=0 FALSE
  all(Tmat %*% as.matrix(Adata[,c(1:5)])==0)      ##TMa!=0   FALSE
  ...
  BPH <- Tmat %*% y
  ## cat("Summary BPH of",TraitName,"with length =",length(BPH),":\n");summary(BPH[,1])
  y_wave <- VinvTmat %*% y;
  
  Ka_wave <- Vinv %*% Tmat %*% KinMatlist$K_a %*% t(Tmat) %*% t(Vinv)
  ...
  Ka_wave2 <- Ka_wave/mean(diag(Ka_wave))
  ...
  if(P3D==TRUE){
    ...
    Kc_wave <- lamdapar[1]*Ka_wave2 + lamdapar[2]*Kd_wave2 + 
      lamdapar[3]*Kaa_wave2 + lamdapar[4]*Kad_wave2 + lamdapar[5]*Kdd_wave2
